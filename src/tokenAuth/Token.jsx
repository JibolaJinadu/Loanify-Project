...
const Token = () => {
  ...
  const handleTokenChange = (index, event) => {
    ...
    if (event.target.value !== '' && index < tokenDigits.length - 1) {
      const nextInputField = event.target.parentElement.nextSibling.querySelector('input');
  ...
  const handleSubmit = async (event) => {
    ...
    try {
      ...
      const response = await axios.post("https://my-json-server.typicode.com/tundeojediran/contacts-api-server/inquiries", { token });
    ...
    <main className='main-container'>
      ...
      <div className='page-content'>
        ...
        <form className='token-section' onSubmit={handleSubmit}>
          <div>
            <h6 className='page-prompt-hdr'>Please enter your 4-digit token pin</h6>
          </div>
          ...
          <div className='button-div'>
            <button type='submit' className='submit-token-btn'>Proceed to the Dashboard</button>
          </div>
